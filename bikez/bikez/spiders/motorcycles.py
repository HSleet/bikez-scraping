...
class MotorcyclesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        all_brands_even = response.selector.xpath(
            "//td[@class='even']//a/@href").getall()
        all_brands_odd = response.selector.xpath(
            "//td[@class='odd']//a/@href").getall()
        all_brands = all_brands_even + all_brands_odd
        yield from response.follow_all(all_brands, self.parse_models)

    def parse_models(self, response):
        all_bikes_list_url = response.selector.xpath(
            "//*[contains(text(), 'list all')]/@href").get()
        yield response.follow(all_bikes_list_url, self.parse_all_bikes)

    def parse_all_bikes(self, response):
        all_bikes_odd = response.selector.xpath(
            "//table[@class='zebra']//tr[@class='odd']//a/@href").getall()[::2]
        all_bikes_even = response.selector.xpath(
            "//table[@class='zebra']//tr[@class='even']//a/@href").getall()[::2]
        all_bikes = all_bikes_odd + all_bikes_even
        yield from response.follow_all(all_bikes, self.parse_bike)

        pagination_buttons = response.selector.xpath(
            "//table[@class='zebra']//td[@colspan='3']//a").getall()
        if len(pagination_buttons) > 0:
            next_page_button_text = response.selector.xpath(
                "//table[@class='zebra']//td[@colspan='3']//a/text()").getall()[-1]
            if 'next' in next_page_button_text.lower():
                next_page_url = response.selector.xpath(
                    "//table[@class='zebra']//td[@colspan='3']//a/@href").getall()[-1]
    ...
    def parse_bike(self, response):
        ...
        def join_text(text_list):
            return " ".join([normalize_spaces(text)
                            for text in text_list]).strip()
        ...
        if len(tables) < 1:
            ...
            bike_specs_dict = extract_specs(specs_table)
            bike_profile = response.xpath(
                "//*[contains(text(), 'profilation')]/../../..//tr[2]//text()").get()
            yield {
                'model': bike_model,
                'brand': bike_brand,
                'year': bike_year,
